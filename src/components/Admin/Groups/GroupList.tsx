...
const GroupList: React.FC = () => {
    ...
    const renderContent = () => {
        ...
        const canDeleteGroup = (g: Group) => (g.apiCount ?? 0) === 0 && (g.userCount ?? 0) === 0;
        ...
        return (
            <>
                ...
                <div className="md:hidden space-y-3">
                    {filtered.map(g => (
                        <div key={g.id} className="bg-white border border-gray-200 rounded-lg shadow-sm p-4 space-y-3">
                            <div>
                                <span className="font-mono text-xs text-gray-700 bg-gray-100 px-2 py-0.5 rounded">{g.code}</span>
                                <p className="text-sm font-medium text-gray-900 mt-1">{g.name}</p>
                            </div>
                            <p className="text-xs text-gray-500">
                                {g.apiCount ?? 0} APIs · {g.userCount ?? 0} Users{g.createdAt ? ` · ${new Date(g.createdAt).toLocaleDateString()}` : ''}
                            </p>
                            <div className="flex gap-2 border-t border-gray-100 pt-3">
                                <button
                                    onClick={() => navigate(`/admin/groups/${g.id}`)}
                                    className="flex-1 text-center text-sm font-medium text-blue-700 bg-blue-50 hover:bg-blue-100 rounded-md py-2 transition-colors"
                                >
                                    Manage
                                </button>
                                <button
                                    onClick={() => openDeleteModal(g)}
                                    disabled={!canDeleteGroup(g)}
                                    className={`flex-1 text-center text-sm font-medium rounded-md py-2 transition-colors ${!canDeleteGroup(g) ? 'text-gray-400 bg-gray-50 cursor-not-allowed' : 'text-red-700 bg-red-50 hover:bg-red-100'}`}
                                    title={!canDeleteGroup(g) ? 'Remove all APIs and users before deleting' : 'Delete group'}
                                >
                                    Delete
                                </button>
                            </div>
                ...
                <div className="hidden md:block overflow-x-auto shadow rounded-lg border">
                    <table className="w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                            <tr>
                                <th
                                    className="px-3 md:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer select-none"
                                ...
                                <th className="px-3 md:px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">APIs</th>
                                <th className="px-3 md:px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Users</th>
                                <th className="px-3 md:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Created</th>
                                <th className="relative px-3 md:px-6 py-3"><span className="sr-only">Actions</span></th>
                            </tr>
                        </thead>
                        <tbody className="bg-white divide-y divide-gray-200">
                            {filtered.map(g => (
                                <tr key={g.id} className="hover:bg-gray-50">
                                    <td className="px-3 md:px-6 py-4 whitespace-nowrap font-mono text-xs text-gray-700">{g.code}</td>
                                    <td className="px-3 md:px-6 py-4 whitespace-nowrap text-sm text-gray-900">{g.name}</td>
                                    <td className="px-3 md:px-6 py-4 whitespace-nowrap text-sm text-gray-700 text-center">{g.apiCount ?? '-'}</td>
                                    <td className="px-3 md:px-6 py-4 whitespace-nowrap text-sm text-gray-700 text-center">{g.userCount ?? '-'}</td>
                                    <td className="px-3 md:px-6 py-4 whitespace-nowrap text-sm text-gray-500">{g.createdAt ? new Date(g.createdAt).toLocaleDateString() : '-'}</td>
                                    <td className="px-3 md:px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-3">
                                        <button onClick={() => navigate(`/admin/groups/${g.id}`)} className="text-blue-600 hover:text-blue-900">Manage</button>
                                        <button
                                            onClick={() => openDeleteModal(g)}
                                            className={`${!canDeleteGroup(g) ? 'text-gray-400 cursor-not-allowed' : 'text-red-600 hover:text-red-800'}`}
                                            title={!canDeleteGroup(g) ? 'Remove all APIs and users before deleting' : 'Delete group'}
                                        >Delete</button>
                                    </td>
                                </tr>
