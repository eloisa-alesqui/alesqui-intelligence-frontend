        ...
        <div style={{ paddingLeft: `${level * 24}px` }} className="text-sm border-t border-gray-100 py-2">
            <div className={`flex items-start ${isExpandable ? 'cursor-pointer' : ''}`} onClick={() => isExpandable && setIsOpen(!isOpen)}>
                ...
                <div className="w-full">
                    ...
                    {propData.example !== undefined && <div className="text-xs mt-1"><span className="text-gray-500">Example: </span><code className="bg-gray-100 p-1 rounded break-all max-w-full inline-block">{JSON.stringify(propData.example)}</code></div>}
