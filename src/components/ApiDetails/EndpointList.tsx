    ...
    return (
        <div className="relative">
            ...
            <button
                ...
                className="absolute top-2 right-2 p-2 bg-gray-700 hover:bg-gray-600 rounded min-w-[44px] min-h-[44px] flex items-center justify-center"
            >
            ...
            <div className="p-4 bg-white">
                ...
                {activeTab === 'schema' && schemaDefinition && (
                    ...
                    schemaRef ? (
                        ...
                        <pre className="text-sm bg-gray-100 p-3 rounded-md overflow-x-auto"><code>{JSON.stringify(schemaDefinition, null, 2)}</code></pre>
                    )
                )}
            </div>
        </div>
                        ...
                        {endpoints.map((endpoint) => {
                            ...
                            return (
                                <div key={endpointId} className={`rounded-lg shadow-sm overflow-hidden transition-all ${isExpanded ? `border-l-4 ${bgColorClass}` : 'border border-gray-200'}`}>
                                    <div className="flex items-center p-3 cursor-pointer hover:bg-gray-50 min-w-0" onClick={() => toggleEndpoint(endpointId)}>
                                        <span className={`w-20 flex-shrink-0 text-center font-bold text-sm py-1 rounded ${methodColors[endpoint.method] || 'bg-gray-500 text-white'}`}>{endpoint.method}</span>
                                        <span className="ml-4 font-mono text-sm flex-grow truncate min-w-0">{endpoint.path}</span>
