...
const Notifications: React.FC = () => {
    ...
    return (
        <div className="fixed top-20 inset-x-4 sm:inset-x-auto sm:right-4 z-50 space-y-2">
            {notifications.map((notification: Notification) => {
                const styles = notificationStyles[notification.type] || notificationStyles.info;
                return (
                    <div
                        key={notification.id}
                        className={`p-4 rounded-lg shadow-lg w-full sm:max-w-sm flex items-center ${styles.bg} ${styles.text} ${styles.border}`}
                    >
                        {styles.icon}
