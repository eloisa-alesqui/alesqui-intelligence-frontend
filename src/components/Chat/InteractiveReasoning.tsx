...
const JsonViewer: FC<JsonViewerProps> = ({ jsonString, title, toolName, response }) => {
    ...
    return (
        <div className="my-2">
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                    <button
                        ...
                        className="inline-flex items-center gap-1 rounded-md border border-gray-300 bg-white px-2 py-1 text-[11px] font-medium text-gray-600 hover:bg-gray-50 hover:text-gray-800 touch-target-44"
            ...
            <div className={`mt-1 rounded-lg bg-gray-900 overflow-hidden max-h-60 overflow-y-auto custom-scrollbar ${wrapMode ? 'overflow-x-hidden' : 'overflow-x-auto'} p-4`}>
                {isPlainString ? (
...
const ToolCallModal: FC<ToolCallModalProps> = ({ toolCall, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50 p-4" onClick={onClose}>
            <div className="bg-white p-4 sm:p-6 rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto custom-scrollbar" onClick={e => e.stopPropagation()}>
                <div className="flex justify-between items-center mb-4">
                    <h3 className="text-base sm:text-lg font-bold text-gray-800 flex items-center min-w-0"><Zap className="w-5 h-5 mr-2 text-blue-600 flex-shrink-0"/> Tool Call: {toolCall.toolName}</h3>
                    <button onClick={onClose} className="text-gray-400 hover:text-gray-600 p-2 min-w-[44px] min-h-[44px] flex items-center justify-center"><X className="w-5 h-5"/></button>
                </div>

                <div className="flex flex-wrap items-center gap-2 sm:gap-4 mb-4 p-2 sm:p-3 bg-gray-50 rounded-md border">
    ...
    return (
        <>
            ...
            <div className="mt-4 border-t border-gray-200 pt-3">
                <button onClick={onToggle} aria-expanded={isExpanded} className="flex items-center space-x-2 text-sm text-gray-600 hover:text-gray-800 w-full text-left touch-target-44">
                ...
                >
                    <div ref={contentInnerRef} className="p-2.5 sm:p-4 bg-blue-50/60 border border-blue-200 rounded-lg space-y-4 sm:space-y-5">
                            ...
                            if (step.type === StepType.THOUGHT) {
                                ...
                                return (
                                    <div key={step.id} className="thought-block">
                                        ...
                                        <div className="pl-3 sm:pl-6 text-gray-700 text-sm italic border-l-2 border-blue-200 ml-1 sm:ml-2">
                                            {planText}
                                        </div>
                                        {actionDescription && (
                                            <div className="tool-call-block mt-4">
                                                ...
                                                <div className="pl-3 sm:pl-6 text-gray-700 text-sm ml-1 sm:ml-2">
                                                    {actionDescription}
                                                </div>
                                            </div>
                                        )}
                            ...
                            if (step.type === StepType.TOOL_CALL && step.toolCalls) {
                                ...
                                return (
                                    <div key={step.id} className="tool-call-block">
                                        ...
                                        <div className="pl-2 sm:pl-6 ml-1 sm:ml-2 space-y-2">
                                            {step.toolCalls.map((toolCall: ToolCallData, index: number) => {
                                                ...
                                                return (
                                                    <button
                                                        ...
                                                        className="w-full flex items-center justify-between text-left p-2.5 sm:p-3 bg-white border border-gray-300 rounded-md hover:bg-gray-50 hover:border-blue-500 transition-all"
                                                    >
                                                        <div className="flex items-center space-x-2 sm:space-x-3 min-w-0">
                                                            {isSuccess ? <CheckCircle className="w-5 h-5 text-green-500 flex-shrink-0" /> : <XCircle className="w-5 h-5 text-red-500 flex-shrink-0" />}
                                                            <div className="text-sm truncate">
                                                                <span className="font-semibold text-gray-800">{toolCall.toolName}</span>
                                                                <span className="text-gray-500 ml-1 sm:ml-2">({executionTime} ms)</span>
                                                            </div>
                                                        </div>
                                                        <div className="flex items-center space-x-1 sm:space-x-2 text-xs flex-shrink-0 ml-2">
