...
interface ChatMessagesProps {
    ...
    onExampleClick: (query: string) => void; // Handler for tapping example queries
}
...
const CodeBlock: FC<any> = ({ node, inline, className, children, ...props }) => {
    ...
    if (!inline && language) {
        return (
            <div className="my-4 rounded-lg overflow-hidden [&_pre]:overflow-x-auto [&_pre]:[-webkit-overflow-scrolling:touch]">
                <div className="bg-gray-800 text-gray-300 px-4 py-2 text-xs flex justify-between items-center"><span className="capitalize">{language}</span><button onClick={() => copyToClipboard(String(children))} title="Copy code" className="p-2 flex items-center justify-center touch-target-44">📋</button></div>
...
const MessageBubble: FC<MessageBubbleProps> = ({ message, index, isReasoningExpanded, onToggleReasoning, hasTechnicalAccess, onOpenReportModal }) => {
    ...
    return (
        <div className={`group flex ${isUser ? 'justify-end' : 'justify-start'}`}>
            ...
            <div className={`max-w-[85vw] sm:max-w-2xl lg:max-w-4xl px-3 sm:px-4 py-2.5 sm:py-3 rounded-lg ${isUser ? 'bg-blue-600 text-white' : message.isError ? 'bg-red-50 text-red-900 border border-red-200' : 'bg-gray-100 text-gray-900'}`}>
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="break-words">
                            {isUser ? (
                                ...
                                <div className="text-white leading-relaxed whitespace-pre-wrap text-sm sm:text-base">{renderTextWithUrls(message.content || '')}</div>
                            ) : (
                        ...
                        <div className="flex justify-between items-center mt-2">
                            {canReport ? (
                                <button onClick={() => onOpenReportModal(message)} className="flex items-center text-xs text-gray-400 hover:text-red-600 opacity-0 group-hover:opacity-100 transition-opacity touch-target-44 touch-visible" title="Report an issue with this response">
                            ...
                            <div className="flex items-center gap-2">
                                <button
                                    ...
                                    className={`opacity-0 group-hover:opacity-100 transition-opacity touch-visible p-1 rounded hover:bg-black/5 flex items-center justify-center touch-target-44 ${isUser ? 'text-blue-200 hover:text-white' : 'text-gray-400 hover:text-gray-600'}`}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
            <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-lg">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <button onClick={onClose} className="text-gray-400 hover:text-gray-600 p-2 min-w-[44px] min-h-[44px] flex items-center justify-center">
                        <X className="w-5 h-5" />
                    </button>
                </div>
                ...
                <div className="bg-gray-100 p-3 rounded-md border border-gray-200 max-h-32 overflow-y-auto mb-4 custom-scrollbar">
...
// =============================================================================
const ChatMessages: FC<ChatMessagesProps> = ({ chatMessages, configuredApis, isLoading, statusMessage, hasTechnicalAccess, onExampleClick }) => {
    // Refs for scrolling
    ...
    const EmptyState = () => (
        <div className="text-center text-gray-500 mt-8">
            ...
            <p className="text-sm mt-1">{hasTechnicalAccess ? "Ask me about your configured APIs." : "Ask me about your available data."}</p>
            {configuredApis.length === 0 && (
                <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg inline-flex">
                    <AlertCircle className="w-4 h-4 text-yellow-600 mr-2" />
                    <p className="text-xs text-yellow-700">{hasTechnicalAccess ? "Configure some APIs first to start chatting!" : "Data sources are not available yet."}</p>
                </div>
            )}
            <div className="lg:hidden mt-6 max-w-md mx-auto text-left">
    ...
    return (
        ...
        <div ref={containerRef} onScroll={handleScroll} className="flex-1 overflow-y-auto p-2 sm:p-4 space-y-3 sm:space-y-4 custom-scrollbar">
