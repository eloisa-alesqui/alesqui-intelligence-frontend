...
const DiagnosticsTab: React.FC = () => {
    ...
    const renderContent = () => {
        ...
        const renderStatusBadge = (ticket: DiagnosticTicketDTO) => {
        ...
        return (
            <>
                ...
                <div className="md:hidden space-y-3">
                    {tickets.map((ticket) => (
                        <div key={ticket.recordId} className="bg-white border rounded-lg shadow-sm p-4 space-y-2">
                            ...
                            <div className="text-sm text-gray-700 font-medium truncate">
                                <User className="w-3.5 h-3.5 mr-1 inline text-gray-400" />
                                {ticket.username}
                            </div>
                            <p className="text-sm text-gray-900 truncate" title={ticket.userPrompt}>
                                <MessageSquare className="w-3.5 h-3.5 mr-1 inline text-gray-400" />
                                {ticket.userPrompt}
                            </p>
                            {ticket.userFeedbackComment && (
                                <p className="text-xs text-gray-600 truncate italic" title={ticket.userFeedbackComment}>
                                    "{ticket.userFeedbackComment}"
                                </p>
                            )}
                            <button
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 space-y-6">
            ...
            >
                ...
                <button
                    type="submit"
                    title="Apply Filters"
                    className="flex items-center justify-center p-2.5 h-10 w-full md:w-10 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 md:ml-auto flex-shrink-0"
                >
